...
export default function Game() {
  ...
  const { state, joinRoom, playCard, unplayCard, resolutionChoice, endDiscussion } = useGameRoom(roomCode);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-1000 ${bgClass}`}>
      <GameBoard state={state} onPlayCard={playCard} onUnplayCard={unplayCard} onResolutionChoice={resolutionChoice} onEndDiscussion={endDiscussion} />
    </div>
  );
}
